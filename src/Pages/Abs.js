...
function Abs() {
  let warmUpElements = retrieveExercises("?subtype=zagrijavanje&type=trbuh");
  let limberUpElements = retrieveExercises("?subtype=razgibavanje&type=trbuh");
  let strength = retrieveDistinctSubTypes("?type=trbuh");
  let stretchUpElements = retrieveExercises("?subtype=rastezanje&type=trbuh");
  const img =
      ...
      response.data.forEach((item, index) => {
        elements.push(
          <Collapsible trigger={item} key={index}>
            {retrieveExercises(`?subtype=snaga-${item}&type=trbuh`)}
          </Collapsible>
        );
      });
    })
