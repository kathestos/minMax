...
function Legs() {
  let warmUpElements = retrieveExercises("?subtype=zagrijavanje&type=noge");
  let limberUpElements = retrieveExercises("?subtype=razgibavanje&type=noge");
  let strength = retrieveDistinctSubTypes("?type=noge");
  let stretchUpElements = retrieveExercises("?subtype=rastezanje&type=noge");
  const img =
      ...
      response.data.forEach((item, index) => {
        elements.push(
          <Collapsible trigger={item} key={index}>
            {retrieveExercises(`?subtype=snaga-${item}&type=noge`)}
          </Collapsible>
        );
      });
    })
